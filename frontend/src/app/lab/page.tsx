...
function Lab({ ix }: { ix: CatalogIndex }) {
  ...
  // stage 1 = baked sponge, 2 = + cream, 3 = + toppings
  const L = base.layers[0]!;
  const staged: CakeDesign = { ...base, layers: [{ ...L, frosting: anim.stage >= 2 ? L.frosting : null, toppings: anim.stage >= 3 ? L.toppings : [] }] };
  ...
  useEffect(() => {
    ...
    const steps: Record<string, [Anim, number]> = {
      bake: [{ type: 'rise', layer: 0 }, 1], cream: [{ type: 'frost', layer: 0 }, 2], top: [{ type: 'top', layer: 0, k: 4 }, 3],
    };
