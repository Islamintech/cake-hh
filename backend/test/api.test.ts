...
import type { DB } from '../src/models/database.js';

// Response bodies are asserted field by field, so they are loosely typed here.
...
async function call(method: string, path: string, { body, key, headers = {} }: { body?: unknown; key?: string; headers?: Record<string, string> } = {}): Promise<{ status: number; json: Json; res: Response }> {
...
test('bakeries filter by dietary options', async () => {
  ...
  assert.deepEqual(halal.json.bakeries.map((b: { id: string }) => b.id), ['s1', 's2']);
  assert.ok(halal.json.bakeries.find((b: { id: string }) => b.id === 's2').notice);
  assert.equal((await call('GET', '/bakeries?options=bogus')).status, 400);
  const one = await call('GET', '/bakeries/s1?options=vegan');
  assert.match(one.json.menu.batters.find((x: { id: string }) => x.id === 'vanilla').locked, /milk/);
  assert.equal(one.json.menu.batters.find((x: { id: string }) => x.id === 'oat').locked, null);
...
test('full order lifecycle: place, track, bakery updates, live stream', async () => {
  ...
  let buf = '';
  const waitFor = async (pred: (b: string) => boolean) => {
    for (;;) {
      if (pred(buf)) return;
  ...
  assert.deepEqual(tracked.json.order.history.map((h: { status: string }) => h.status), ['received', 'accepted', 'baking', 'ready', 'pickedup']);
...
test('customer tracking stream pushes status changes', async () => {
  ...
  const dec = new TextDecoder();
  let buf = '';
  const waitFor = async (s: string) => { while (!buf.includes(s)) buf += dec.decode((await reader.read()).value); };
