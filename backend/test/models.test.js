...
import { localSuggest, sanitizeSuggestions, createSuggester } from '../src/services/suggestionService.js';

const opts = (...ids) => normalizeOptions(ids).opts;
const cake = (over = {}) => ({
...
test('bakery fit: halal excludes s3, vegan excludes s3 (no vegan batter there)', () => {
  assert.equal(Bakery.fits(Bakery.findById('s3'), opts('halal')), false);
  assert.equal(Bakery.fits(Bakery.findById('s3'), opts('vegan')), false);
  assert.equal(Bakery.fits(Bakery.findById('s1'), opts('vegan')), true);
});
...
test('pricing matches the demo formula', () => {
  ...
  assert.deepEqual(pricing(c, Bakery.findById('s1')), { subtotal: 45500, discount: 4600, total: 40900, currency: 'KRW' });
...
test('validateCake rejects unstocked, locked, unknown and over-limit designs', () => {
  ...
  assert.match(validateCake(cake({ layers: three }), Bakery.findById('s2'), opts()).errors[0], /up to 2 layers/);
...
test('validateCake normalizes: clamps positions, strips markup, marks baked', () => {
  const v = validateCake(cake({ lettering: ' <b>Hi</b> ', layers: [{ batter: 'vanilla', frosting: 'whip', toppings: [{ id: 'straw', fx: 9, fy: 'x' }], baked: false }] }), Bakery.findById('s1'), opts());
...
test('Claude path: sends enum-constrained schema, sanitizes output, falls back on refusal/error', async () => {
  ...
  const fake = (res) => ({ messages: { create: async (body) => { sent = body; if (res instanceof Error) throw res; return res; } } });
  ...
  const ok = await createSuggester({}, silent, fake(reply('end_turn', json))).suggest({ cravings: ['fruity'], text: 'hi' }, opts('vegan'));
  ...
  const item = sent.output_config.format.schema.properties.suggestions.items.properties;
  assert.deepEqual(item.batter.enum, ['oat']);
  assert.ok(!item.toppings.items.enum.includes('cookie'));
  assert.match(sent.messages[0].content, /<customer_note>hi<\/customer_note>/);

  for (const res of [reply('refusal', ''), reply('end_turn', 'not json'), new Error('network down')]) {
    const r = await createSuggester({}, silent, fake(res)).suggest({ cravings: [] }, opts());
