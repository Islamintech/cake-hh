...
import { partnerSchema, quoteSchema, suggestionSchema } from '../validators/schemas.js';
import type { RouteDeps } from './index.js';

/** Catalog, bakeries, quotes, AI suggestions and partnership applications. No auth. */
export function publicRoutes({ config, controllers }: RouteDeps): Router {
  ...
  r.post('/partners', rateLimiter(config, 5), validateBody(partnerSchema), controllers.partner.create);
  return r;
}
